...
async function main() {
  ...
  const royal = await db.collection('tenants').insertOne({
    ...
    theme: {
      ...
      bgColor: '#fafaf7',
      textColor: '#1a1d24',
      logoUrl: 'https://placehold.co/200x80/1a4d8c/d4a85a?text=ROYAL+LYON',
      font: 'Playfair Display',
    },
    contact: {
      phone: '04 78 28 00 00',
      email: 'reception@royal-lyon.fr',
  ...
  const azur = await db.collection('tenants').insertOne({
    ...
    theme: {
      ...
      bgColor: '#f8fafc',
      textColor: '#0f172a',
      logoUrl: 'https://placehold.co/200x80/0e7490/fbbf24?text=COTE+AZUR',
      font: 'Cormorant Garamond',
    },
    contact: {
      phone: '04 93 88 00 00',
      email: 'info@cote-azur.fr',
  ...
  const lyonPois = [
    ...
    { name: { fr: 'Parc de la Tête d\'Or' }, category: 'park', lat: 45.7783, lng: 4.8520, rating: 4.8 },
    ...
    { name: { fr: 'Musée des Confluences' }, category: 'museum', lat: 45.7320, lng: 4.8189, rating: 4.5 },
    { name: { fr: 'Le Pain & le Vin' }, category: 'bar', lat: 45.7600, lng: 4.834, rating: 4.5 },
    { name: { fr: 'Pharmacie Bellecour' }, category: 'pharmacy', lat: 45.7575, lng: 4.831 },
  ];
  await db.collection('pois').insertMany(
    lyonPois.map((p) => ({ ...p, tenantId: tenants[0].id, createdAt: new Date() })),
  );

  const azurPois = [
    { name: { fr: 'La Petite Maison' }, category: 'restaurant', lat: 43.6962, lng: 7.2680, rating: 4.7 },
    { name: { fr: 'Promenade des Anglais' }, category: 'monument', lat: 43.6929, lng: 7.2630, rating: 4.8 },
    ...
    { name: { fr: 'MAMAC' }, category: 'museum', lat: 43.7011, lng: 7.2780, rating: 4.4 },
    { name: { fr: 'Aéroport Nice' }, category: 'transport', lat: 43.6584, lng: 7.2159 },
  ];
  await db.collection('pois').insertMany(
    azurPois.map((p) => ({ ...p, tenantId: tenants[1].id, createdAt: new Date() })),
  );
  ...
  for (const t of tenants) {
    ...
    const samples = Array.from({ length: 50 }, (_, i) => {
      ...
      return {
        ...
        locale: 'fr',
        metadata: { device: 'kiosk-checkout', room: String(100 + (i % 30)) },
  ...
  console.log('   admin@royal-lyon.fr — admin Royal Lyon');
  console.log('   staff@royal-lyon.fr — staff Royal Lyon');
  console.log('   admin@cote-azur.fr — admin Côte d\'Azur');
